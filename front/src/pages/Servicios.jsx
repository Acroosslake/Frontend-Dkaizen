              ...
              <AnimatePresence>
                {isMenuOpen && (
                  ...
                  >
                    ...
                    {user.role === 'barber' && (
                      <Link to="/barber-dashboard" className="block px-5 py-3 text-[10px] uppercase tracking-widest text-dk-gold font-black hover:bg-gray-900 transition-colors border-b border-gray-900/50">Mi Agenda (Staff)</Link>
                    )}
          ...
          <div className="bg-[#0a0a0a] border border-gray-900 rounded-[3rem] overflow-hidden shadow-2xl relative group">
            ...
            <div className="p-6 md:p-8 relative z-20 -mt-32">
              <div className="bg-[#030303]/80 backdrop-blur-2xl border border-gray-800/50 p-8 rounded-[2.5rem] shadow-2xl">
                ...
                {/* 🛡️ VALIDACIÓN DEL BOTÓN DE AGENDAR SEGÚN EL ROL */}
                {user?.role === 'barber' ? (
                  <button disabled className="w-full bg-[#111] text-gray-500 border border-gray-800 cursor-not-allowed font-black py-5 rounded-2xl text-[10px] uppercase tracking-[0.3em] transition-all">
