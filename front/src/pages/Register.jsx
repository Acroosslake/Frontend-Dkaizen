...
function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // Si el backend devuelve el token tras registrarse, lo logueamos de una
      if (response.data.token) {
        await login(response.data.token);
        navigate('/reservas'); // Lo mandamos directo a agendar su primer corte
      } else {
        // Si no devuelve token, lo mandamos al login para que entre manual
        navigate('/login');
      }
    ...
    <div className="min-h-screen bg-[#030303] text-white font-sans flex items-center justify-center px-4 relative overflow-hidden selection:bg-dk-gold selection:text-black py-10">
      ...
      >
        ...
        <AnimatePresence>
          {error && (
            <motion.div 
              ...
              className="bg-red-900/20 border border-red-500/50 text-red-400 px-4 py-3 rounded-2xl mb-6 text-xs text-center font-medium overflow-hidden"
            >
              {error}
            </motion.div>
          )}
        ...
        <form onSubmit={handleRegister} className="space-y-5">
          <div>
            <label className="block text-[9px] uppercase tracking-[0.2em] text-gray-500 font-black mb-2 ml-2" htmlFor="name">
              Nombre Completo
            </label>
            <input
              id="name"
              ...
              className="w-full px-5 py-3.5 bg-[#111111] border border-gray-800 rounded-2xl focus:outline-none focus:border-dk-gold text-sm text-white transition-all placeholder-gray-700"
              placeholder="Ej. David Ramírez"
            />
          </div>

          <div>
            <label className="block text-[9px] uppercase tracking-[0.2em] text-gray-500 font-black mb-2 ml-2" htmlFor="email">
              Correo Electrónico
            </label>
            <input
              id="email"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[9px] uppercase tracking-[0.2em] text-gray-500 font-black mb-2 ml-2" htmlFor="password">
                Contraseña
              </label>
              <input
                id="password"
            ...
            <div>
              <label className="block text-[9px] uppercase tracking-[0.2em] text-gray-500 font-black mb-2 ml-2" htmlFor="passwordConfirmation">
                Confirmar
              </label>
              <input
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-white hover:bg-dk-gold text-black font-black py-4 rounded-2xl text-[10px] uppercase tracking-[0.3em] transition-all duration-300 shadow-[0_0_20px_rgba(255,255,255,0.1)] disabled:opacity-50 mt-2 transform hover:scale-[1.02]"
          >
        ...
        <div className="mt-6">
          <div className="relative">
            ...
            <div className="relative flex justify-center text-[9px] uppercase tracking-[0.2em] font-bold">
              <span className="px-4 bg-[#0a0a0a] text-gray-600">Registro Rápido</span>
            </div>
          </div>
          ...
          <div className="mt-6">
            <button
              ...
              type="button"
              disabled={loading}
              className="w-full flex items-center justify-center space-x-3 bg-transparent border border-gray-700 hover:border-white hover:bg-white hover:text-black text-white font-black py-3.5 rounded-2xl text-[10px] uppercase tracking-[0.2em] transition-all duration-300 disabled:opacity-50"
            >
        ...
        <div className="mt-8 text-center flex flex-col gap-3">
          <p className="text-[10px] uppercase tracking-widest text-gray-500">
            ...
            <Link to="/login" className="text-dk-gold hover:text-white font-bold transition-colors">
              Ingresa aquí
            </Link>
          </p>
